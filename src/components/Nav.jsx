export function Nav({ tab, setTab }) {
  const NAV = [{id:"appointments",icon:"📅",label:"Agenda"},{id:"home",icon:"📊",label:"Scadenze"},{id:"payments",icon:"📋",label:"Riepilogo"},{id:"bills",icon:"📄",label:"Bollette"},{id:"settings",icon:"⚙️",label:"Config"}];
  ...
  return (
    <div style={{background:"linear-gradient(145deg,#162560 0%,#2D4FBD 60%,#3B63E8 100%)",color:"#fff",flexShrink:0,paddingBottom:"env(safe-area-inset-bottom)",position:"relative"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)"}}>
        {NAV.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"8px 0 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:2,border:"none",background:"none",cursor:"pointer",color:tab===t.id?"#fff":"rgba(255,255,255,.45)",transition:"color .2s",position:"relative",WebkitTapHighlightColor:"transparent"}}>
